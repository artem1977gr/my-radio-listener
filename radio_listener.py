...
TOTAL_BASE_WEIGHT = sum(STATION_WEIGHTS.values())
...
def scheduler_manager(active_processes):
    # Фильтруем ТОЛЬКО живые процессы от тех, что уже умерли сами собой
    ...
    if to_spawn := target_count - current_live > 0:
        ...
        for radio_url in spawn_queue[:to_spawn]:
            ...
            p.start()
            # Добавляем НАПРЯМУЮ в живой список
    ...
    #### УДАЛЕНИЕ ЛИШНИХ ПРОЦЕССОВ ####
        ...
        # Удаляем убитые процессы ИЗ живого списка
...
if __name__ == "__main__":
    ...
    # Присваиваем результат менеджеру!
    ...
    try:
        while True:
            ...
            # ГЛАВНАЯ СТРОЧКА: мы берем возвращенный СПИСОК и полностью заменяем им старый
            manager_active = scheduler_manager(manager_active)
